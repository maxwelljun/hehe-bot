...
程序启动时校验策略范围，并根据完整策略生成稳定 ID 参与任务键计算。策略只能在停止监控后修改；存在等待确认、等待结算或状态不明的订单时拒绝切换，避免用新策略继续旧任务。
...
4. 同一桌同一局只允许一个在途订单；全局发送串行化，响应匹配后再发送下一笔，避免响应串单和瞬时频率过高。
...
以下任一情况自动暂停真实模式：登录失效、运行时接口兼容检查失败、全桌快照不完整、消息长时间未更新、CDP 重连、订单响应超时、订单状态未知、验证码 `-140`、余额或限额异常、达到当日上限、配置在运行中改变。
