...
internal static class ChromeLauncher
{
    ...
    public static async Task<bool> IsDebugEndpointReadyAsync(int port, CancellationToken cancellationToken)
    {
        try
        {
            ...
            using HttpResponseMessage response = await client.GetAsync($"http://127.0.0.1:{port}/json/version", cancellationToken).ConfigureAwait(false);
    ...
    public static async Task LaunchAsync(int port, string profileDirectory, CancellationToken cancellationToken)
    {
        ...
        string chrome = FindChrome() ?? throw new FileNotFoundException("未找到 Google Chrome。请先安装 Chrome。", "chrome.exe");
    ...
    public static async Task<IReadOnlyList<Uri>> FindPageTargetsAsync(int port, CancellationToken cancellationToken)
    {
        ...
        using Stream stream = await client.GetStreamAsync($"http://127.0.0.1:{port}/json/list", cancellationToken).ConfigureAwait(false);
        using JsonDocument document = await JsonDocument.ParseAsync(stream, cancellationToken: cancellationToken).ConfigureAwait(false);
        var targets = document.RootElement.EnumerateArray().Select(element => new
        {
        ...
        return targets.Where(item => item.Type == "page"
                && Uri.TryCreate(item.Url, UriKind.Absolute, out Uri? uri)
                && (uri.Scheme == Uri.UriSchemeHttps || uri.Scheme == Uri.UriSchemeHttp)
                && item.WebSocket is not null)
            .OrderByDescending(item => Uri.TryCreate(item.Url, UriKind.Absolute, out Uri? uri) && IsYaxinHost(uri.Host))
    ...
    private static bool IsYaxinHost(string host) =>
        host.Equals("yaxin868.com", StringComparison.OrdinalIgnoreCase)
        || host.EndsWith(".yaxin868.com", StringComparison.OrdinalIgnoreCase);
